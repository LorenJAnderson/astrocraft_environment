...
from .constants import MAX_FUEL, PTACT, PTCOL, PTMAX, R0, MIN_FUEL_DICT, SLOW_FUEL_COSTS
...
class CTFENVMA(ParallelEnv):
    ...
    def step(self, actions):
        ...
        1:      Transfer to Orbital -3 
        2:      Transfer to along Orbital -2 
        3:      Transfer to Orbital -1 
        4:      Transfer to Orbital +0 
        5:      Transfer to Orbital +1 
        6:      Transfer to Orbital +2 
        7:      Transfer to Orbital +3 

        8:      Slow intercept Own Base (should be masked if craft is unflagged)
        9:      Fast intercept Own Base (should be masked if craft is unflagged)

        10+2*k: Slow intercept opponent agent k
        11+2*k: Fast intercept opponent agent k

        NOTE: Even numbers that aren't transfers will always be slow intercepts, while odd numbers are fast intercepts
    ...
    def render(self):
        """Produces a visual representation of the current game state (i.e. a single frame)"""
    ...
    def reward(self, agent):
        """
        This method assigns the reward per timestep dependent on the state of the system. It also determines whether the game has reached a terminal state, or been truncated.
        ...
        # If no flag returns happened, check to make sure both teams can still play        
